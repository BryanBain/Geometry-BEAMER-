...
\onslide<2->{\tkzFillPolygon[blue!40, opacity=0.6](A,D,C,B)}
\onslide<3->{\tkzFillPolygon[green!40, opacity=0.3](B,C,G,F)}
\onslide<4->{\tkzDrawSegment[ultra thick](B,C)}
\end{tikzpicture}
\end{center}

\onslide<4->{$\overleftrightarrow{BC}$}
\end{frame}

\begin{frame}{Naming Planes}
When naming planes with 4 or more points, list the points in order either clockwise or counterclockwise.
\end{frame}

\begin{frame}{Example 4}
(a)	\quad What plane contains $N, \, P, \text{ and } Q?$
\begin{center}
\begin{tikzpicture}
	...
	\onslide<2->{\tkzFillPolygon[blue!40, opacity=0.4](N,P,Q,R)}
\end{tikzpicture}
\end{center}
\onslide<3->{plane $NPR$}
\end{frame}
...
\begin{frame}{Example 4}
(b)	\quad What plane contains $J$, $M$, and $Q$?
\begin{center}
\begin{tikzpicture}
	\tkzDefPoints{0/0/N, 4/0/P, 4/2/K, 0/2/J}
	\tkzDrawPolygon(N,P,K,J)
	...
	\tkzLabelPoints[above right](R)
	\tkzLabelPoints[right](P,Q)
	\onslide<2->{\tkzFillPolygon[blue!40, opacity=0.4](J,M,Q,P)}
\end{tikzpicture}
\end{center}
\onslide<3->{plane $JMQ$}
\end{frame}
